...
std::vector<std::pair<uint32_t, uint32_t>> undirected_edges(const mesh& obj)
{
	...
	auto add_edge = [&](uint32_t a, uint32_t b) { edges.emplace_back(std::min(a, b), std::max(a, b)); };	// edge的两个vertex，Index小的在前，大的在后
	for (size_t t = 0; t < obj.triangles.size(); t++)		   // 将三个边都加进去
	{
...
{
	const auto [v1, v2] = connectivity.edge_vertices(h);		// h是 v1 -> v2
	V.push_back(v1);
	V.push_back(v2);
	...
	auto add_vert_ring1 = [&](uint32_t vert)
	{
		...
		while (he.is_valid() && he.index != index)				// 将所有从vert射出的halfedge的vertex推入V （除了v1 v2这两个vertex），其所在的三角形推入F
		{
