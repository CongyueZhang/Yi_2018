...
private:
	struct range_vertex_ring
	{
		...
		struct iterator
		{
			...
			iterator& operator++()							//初始half_edge的出射点为中心（假设为u），遍历u出射的所有halfedge							
			{												//注意half_edge类中的vertex()得到的是该halfedge的入射点
				he = he.next();
				current = he.vertex();						//current也代表了当前he的index
				he = he.next().opposite();					//得到的新he是 u -> current
		...
		std::vector<uint32_t> to_vector() const
		{
			...
			for (uint32_t v : *this)				//遍历begin和end之间的iterator的current
				vec.push_back(v);
			return vec;
		}
	};
...
public:
	...
	/// 增加一个默认构造函数
	half_edge_connectivity() = default;
...
// inline bool half_edge::is_delaunay_valid() const {return index != uint32_t(-1) && connec->half_edges[index].delaunay_valid;}
...
inline std::array<uint32_t, 3> half_edge_connectivity::triangle_half_edges(uint32_t h) const		// 返回h所在三角形的三个halfedge的index
{
...
// 实际传递进来的begin是指向triangle vector 第一个data的指针
template<typename It>
inline std::vector<std::pair<uint32_t, uint32_t>> half_edge_connectivity::create_loops(It begin, It end)
{
	...
	while (begin != end)
	{
		...
		half_edges.emplace_back(i1, h + 1);			//i1是此vertex，h+1是next
		half_edges.emplace_back(i2, h + 2);
		half_edges.emplace_back(i0, h + 0);
		vh.emplace_back(i0, h + 0);					//每个顶点及其所连的两个edge
